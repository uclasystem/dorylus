             ...
             void* context)
{
    ...
    if (type == ZOO_SESSION_EVENT) {
      ...
      fprintf(stderr, "keval! something got deleted in %s\n", path);
      if(context == NULL)
        fprintf(stderr, "context is null\n");
      else
        fprintf(stderr, "answer to earth, universe and everything is %d\n", *((int*) context));
    }
    else if(type == ZOO_CHILD_EVENT) {
	fprintf(stderr, "keval! something happened [creation or deletion] in %s\n", path);
	}
}
...
void my_strings_completion(int rc, const struct String_vector *strings,
    ...
    usec = tv.tv_usec - startTime.tv_usec;
    fprintf(stderr, "time = %d msec\n", sec*1000 + usec/1000);
    fprintf(stderr, "kdata: %s: rc = %d\n", (char*)data, rc);
    if (strings)
        for (i=0; i < strings->count; i++) {
            fprintf(stderr, "kdir%d: \t%s\n", i, strings->data[i]);
        }
    free((void*)data);
    gettimeofday(&tv, 0);
    sec = tv.tv_sec - startTime.tv_sec;
...
//        }
        if (is_create2) {
          ...
          rc = zoo_acreate(zh, line, "", 0, &ZOO_OPEN_ACL_UNSAFE, flags,
        ...
        if (rc) {
            ...
            fprintf(stderr, "Keval! Mostly %s exists!!!\n", line);
        }
...
int main(int argc, char **argv) {
    ...
    hostPort = argv[1];
    zh = zookeeper_init(hostPort, watcher, 1000, &myid, NULL, flags);
    if (!zh) {
        return errno;
    }
