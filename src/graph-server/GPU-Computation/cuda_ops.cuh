...
struct divide_functor
{
    ...
    __host__ __device__
    float operator()(const int &i, const float &x) const
    {
        return x / denoms[i];
    }
    float *denoms;
...
struct setRowStarts
{
    ...
    __host__ __device__
    FeatType *operator()(const unsigned &i) const
    {
...
{
    findRowMaximum(unsigned col_)
        : col(col_) {}
    __host__ __device__
    unsigned operator()(const FeatType *ptr) const
    {
        unsigned index = 0;
        float max = ptr[0];
...
{
    isPredictCorrect(unsigned col_)
        : col(col_) {}
    __host__ __device__
    unsigned operator()(const unsigned pred, const FeatType *label) const
    {
...
struct getLoss
{
    ...
    __host__ __device__
    FeatType operator()(const unsigned &i, const FeatType *softmax_ptr) const
    {
