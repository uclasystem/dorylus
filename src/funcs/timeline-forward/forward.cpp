 ...
 *
 */
static Matrix requestMatrix(zmq::socket_t& socket, OP op, unsigned id,
                            bool data = false) {
    // Send pull request.
...
static void sendMatrices(Matrix& zResult, Matrix& actResult,
    ...
    // Send push header.
    zmq::message_t header(HEADER_SIZE);
    populateHeader((char*)header.data(), OP::PUSH_FORWARD, id,
                   zResult.getRows(), zResult.getCols());
...
static void sendWeightUpdates(zmq::socket_t& socket, Matrix& weightUpdates,
                              unsigned layer) {
    zmq::message_t header(HEADER_SIZE);
    ...
    std::memcpy(updateMsg.data(), weightUpdates.getData(),
                weightUpdates.getDataSize());
 ...
 *
 */
static Matrix dot(Matrix& features, Matrix& weights) {
    unsigned m = features.getRows(), k = features.getCols(),
             n = weights.getCols();
 ...
 *
 */
static Matrix activate(Matrix& mat) {
...
static unsigned checkAccuracy(Matrix& predictions, Matrix& labels) {
    ...
    for (unsigned r = 0; r < predictions.getRows(); ++r) {
        ...
        if (labels.get(r, maxIndex) == 1.0) ++totalCorrect;
    }
 ...
 *
 */
static void evaluateModel(Matrix& activations, zmq::socket_t& datasocket,
                          unsigned partId) {
    Matrix labels = requestMatrix(datasocket, OP::PULL_EVAL, partId);
    Matrix predictions = softmax(activations);

    // Check if the label with the highest probability after softmax is equal to
    // the target label
...
static Matrix gradLoss(Matrix& z, Matrix& weights, Matrix& AH,
                       zmq::socket_t& datasocket, zmq::socket_t& weightSocket,
 ...
 *
 */
static invocation_response forward_prop_layer(std::string dataserver,
    ...
    *(unsigned*)(identity + sizeof(unsigned) * 2) = rand();
    ...
    try {
        auto treq =
            gt.getTimer(std::string("LNREQ->") + "Id" + std::to_string(id) +
        ...
        std::thread t([&] {  // Weight requests run in a separate thread.
            std::cout << "< FORWARD > Asking weightserver at " << whost_port
                      << "..." << std::endl;
            do {
                weights =
                    requestMatrix(weights_socket, OP::PULL_FORWARD, layer);
            } while (weights.empty());
        });

        // Request feature activation matrix of the current layer.
        std::cout << "< FORWARD > Asking dataserver at " << dhost_port << "..."
                  << std::endl;
        do {
        ...
        auto tcomp =
            gt.getTimer(std::string("LC->") + "Id" + std::to_string(id) + "Ly" +
                        std::to_string(layer));
        tcomp->start();
        // Multiplication.
        z = feats.dot(weights);
        ...
        tcomp->stop();
        auto tsend =
            gt.getTimer(std::string("LNSend->") + "Id" + std::to_string(id) +
                        "Ly" + std::to_string(layer));
        tsend->start();
...
static invocation_response my_handler(invocation_request const& request) {
    ...
    return forward_prop_layer(dataserver, weightserver, dport, wport, chunkId,
                              layer, lastLayer);
}
