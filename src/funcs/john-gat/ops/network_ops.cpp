...
int recvTensor(zmq::socket_t& socket, Matrix &mat, unsigned timeout) {
    int recvStatus = 0;
    zmq::message_t tensorHeader(TENSOR_HDR_SIZE);
    zmq::message_t tensorData;

    
    recvStatus = timedRecv(socket, tensorHeader, timeout);
    if (recvStatus == 3) {
        return recvStatus;
    }
    ...
    recvStatus = timedRecv(socket, tensorData, timeout);
    if (recvStatus == 3) {
        return recvStatus;
    }
...
// for requesting a single tensor
void reqTensor(zmq::socket_t& socket, Chunk& chunk, std::string tensorName, bool sndMore) {
    ...
    if (sndMore) socket.send(tensorHeader, ZMQ_SNDMORE);
    else socket.send(tensorHeader);
}

std::vector<Matrix> reqTensors(zmq::socket_t& socket, Chunk &chunk,
                        std::vector<std::string>& tensorRequests, unsigned timeout) {
...
#define EXP_FACTOR 1.5
    ...
    while (true) {
        ...
        while (more && !empty) {
            Matrix result;
            int ret = recvTensor(socket, result, timeout);
            if (ret == -1) {
