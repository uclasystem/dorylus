...
std::vector<char> constructIdentity(unsigned locId, unsigned layer, unsigned dir, std::string& ip) {
    ...
    std::memcpy(identity.data() + (sizeof(unsigned) * 4), ip.c_str(), ip.length() + 1);
...
apply_vertex(zmq::socket_t& data_socket, zmq::socket_t& weights_socket, Chunk &chunk) {
    ...
    int recvStatus;
    Matrix H;
    std::cout << "Sending request to graph server" << std::endl;
    reqTensor(data_socket, chunk, "h");
    recvStatus = recvTensor(data_socket, H, 1500);
    if (recvStatus == 3) {
        std::cout << "Data request timed out" << std::endl;
        return constructResp(false, chunk.localId, "Graph server request timed out");
    }

    Matrix W;
    std::cout << "Sending request to weight server" << std::endl;
    reqTensor(weights_socket, chunk, "w");
    recvStatus = recvTensor(weights_socket, W, 1500);
    if (recvStatus == 3) {
        std::cout << "Weights request timed out" << std::endl;
        return constructResp(false, chunk.localId, "Weight server request timed out");
    }

//    std::vector<Matrix> matrices;
//    // Request H directly
//    std::vector<std::string> dataRequests{"h"};
//    matrices = reqTensors(data_socket, chunk, dataRequests, 1500);
//    for (auto& M : matrices) {
//        if (M.empty()){
//            std::cout << M.name() << " is empty" << std::endl;
//            return constructResp(false, chunk.localId, M.name() + " is empty");
...
//    std::vector<Matrix> weights = reqTensors(weights_socket, chunk, weightRequests, 1500);
//    for (auto& W : weights) {
//        if (W.empty()){
//            H.free();
//            std::cout << W.name() << " is empty" << std::endl;
...
apply_phase(std::string dataserver, std::string weightserver, unsigned dport, unsigned wport, Chunk &chunk, bool eval) {
    ...
    std::cout << "CLIENT " << chunk.str2() << " " << clientIdStr(identity.data()) << std::endl;
...
my_handler(invocation_request const& request) {
    ...
    std::cout << "[ACCEPTED] Thread " << chunk.str() << " is requested from "
              << dataserver << ":" << dport << ", layer " << chunk.layer
