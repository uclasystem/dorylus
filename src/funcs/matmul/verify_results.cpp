...
Matrix dot(Matrix& features, Matrix& weights) {
	DTYPE* zData = new DTYPE[features.cols * features.rows];
	DTYPE* feats = features.getData();

	for (int i = 0; i < features.rows * features.cols; ++i) {
		zData[i] = feats[i] + 1;
	}

	Matrix m(features.rows, features.cols, zData);
	return m;
...
Matrix activate(Matrix& mat) {
	...
	for (int i = 0; i < mat.rows * mat.cols; ++i) {
		activationData[i] = zData[i] + 1; //std::tanh(zData[i]);
	}
