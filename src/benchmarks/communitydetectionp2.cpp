...
class CDInit : public VertexProgram<VertexType, EdgeType> {
  public:
    bool update(Vertex<VertexType, EdgeType>& vertex, EngineContext& engineContext) {
      if(vertex.data() != inits[vertex.globalId()]) {
        vertex.setData(inits[vertex.globalId()]);
        return true;
      }
      return false;
    }
...
int main(int argc, char* argv[]) {
  ...
  char initFile[256];
  parse(&argc, argv, "--cd-initfile=", initFile);
  fprintf(stderr, "InitFile = %s\n", initFile);

  VType defaultVertex = MAX_IDTYPE;
  Engine<VType, EType>::init(argc, argv, defaultVertex);

  loadInitInfo(initFile);
