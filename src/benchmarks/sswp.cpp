...
template<typename VertexType, typename EdgeType>
class SSWPProgram : public VertexProgram<VertexType, EdgeType> {
  public:
    bool update(Vertex<VertexType, EdgeType>& vertex, EngineContext& engineContext) {
      ...
      for(unsigned i=0; i<vertex.numInEdges(); ++i) {
        ...
        maxWidth = std::max(maxWidth, std::min(vertex.getSourceVertexData(i), vertex.getInEdgeData(i)));
      }
      bool changed = (vertex.data() != maxWidth);
...
int main(int argc, char* argv[]) {
  ...
  SSWPProgram<VType, EType> sswpProgram;
  WriterProgram<VType, EType> writerProgram;

  if(reset)
    Engine<VType, EType>::streamRun(&sswpProgram, &writerProgram, &resetComputations, true);
  else
    Engine<VType, EType>::streamRun(&sswpProgram, &writerProgram, NULL, true);
