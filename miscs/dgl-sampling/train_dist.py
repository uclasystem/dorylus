...
Configuration:
    LayerConfig: (300, 64, 25) for Amazon, (301, 128, 50) for Reddit-large
    Learning Rate: 0.003
    Self-loop has been enabled in datasets already
    Fan-out: (10, 25)
    Sample batch size: 50000
    We count training time only (validation time is also pretty long)
...
def run(args, device, data):
    ...
    for epoch in range(args.num_epochs):
        ...
        for step, blocks in enumerate(dataloader):
            ...
            if g.rank() == 0 and step % args.log_every == 0:
