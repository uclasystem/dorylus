r'''
Self-implemented GraphConv module. Use our self-implemented xariver_uniform function.
Disable matrix-matrix multiplication optimization in GraphConv to enable a fair comparison.
'''
...
class GraphConv(nn.Module):
    ...
    def __init__(self,
        ...
        if weight:
            if True: # YIFAN: default weight setting
                self.weight = nn.Parameter(th.Tensor(in_feats, out_feats))
            else:
                if self._in_feats == 602:
                    self.weight = nn.Parameter(th.load('data/layer0.pt'))
                elif self._in_feats == 128:
                    self.weight = nn.Parameter(th.load('data/layer1.pt'))
        else:
    ...
    def ref_forward(self, graph, feat, weight=None):
